...
interface AnalyzeCodeArgs {
  code: string;
  language: string;
  analysis_type: 'security' | 'performance' | 'style' | 'bugs';
}

const isGenerateTextArgs = (args: any): args is GenerateTextArgs =>
  typeof args === 'object' &&
  args !== null &&
...
const isAnalyzeCodeArgs = (args: any): args is AnalyzeCodeArgs =>
  typeof args === 'object' &&
  args !== null &&
        ...
        switch (request.params.name) {
          case 'generate_text': {
            if (!isGenerateTextArgs(request.params.arguments)) {
              throw new McpError(
                ...
                'Invalid generate_text arguments'
              );
            }

            const args = request.params.arguments as GenerateTextArgs;
          ...
          case 'analyze_code': {
            if (!isAnalyzeCodeArgs(request.params.arguments)) {
              throw new McpError(
                ErrorCode.InvalidParams,
                'Invalid analyze_code arguments'
              );
            }

            const args = request.params.arguments as AnalyzeCodeArgs;
